"""
Source: https://github.com/oblique63/Python-GoogleCalendarParser (with a small diff to get it working)
...
class CalendarParser(object):
    ...
    def parse_ics(self, overwrite_events=True):
        ...
        assert self.ics_url or self.ics_file, "No ics resource has been set."  # XXX

        # Returns an icalendar.Calendar object.
        self.fetch_calendar(force_ics=True)

        # self.time_zone = timezone(str(self.calendar["x-wr-timezone"]))
        self.time_zone = timezone('America/Vancouver')  # XXX
        # self.title = str(self.calendar["x-wr-calname"])
        self.title = str(self.calendar.get("x-wr-calname"))  # XXX
